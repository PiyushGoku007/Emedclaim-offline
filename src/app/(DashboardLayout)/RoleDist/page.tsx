...
import { Asterisk, numStyle } from "../components/StylesnS";
...
const RoleDistribution = () => {
      ...
      renderCell: (params) => {
        const handleClick = () => {
          setRows((prevRows: any) =>
            prevRows.map((item: any) =>
              item.email_id === params.row.email_id
                ? { ...item, active_Status: !params.row.active_Status }
                : item
            )
          );
        };
  ...
  const registration = async () => {
    const data = await axios.post(
      `${BACKEND_BASE_URL}/api/medical/createUser`,
      formData
    );
    getData();
    setFormData({
  ...
  const getData = async () => {
    const data = await axios.get(`${BACKEND_BASE_URL}/api/medical/getAllUser`);
  ...
  const getRoles = async () => {
    const data = await axios.get(`${BACKEND_BASE_URL}/api/medical/getAllRole`);
    setRoles(
      data.data.data.map((item: any) => ({ id: item._id, role: item.name }))
    );
  };
